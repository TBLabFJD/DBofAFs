...
# esto es una copia del CLEANED_pipeline_BD_SNV.sh; copiado el 04/09/2026. La he hecho para añadir apuntes para Mar, y algunas lineas nuevas indicadas con 04/09/2026 (son lineas anadidas del https://github.com/TBLabFJD/DBofAFs/blob/db_cancer/FINAL_pipeline_cancer_db_no_plink.sh)
...
#04/09/2026 -> comento esto porque ya esta la 1.21 en uam, lo cargo abajo
...
#04/09/2026: ana amil: 16/06/2025 hemos puesto minconda 3.8 en vez de 3.6 porque supuestamente hail 0.2.120 esta aqui en la 3.8 cargado
#04/09/2026: ana amil: 17/06/2025 al haber incompatibilidad con perl y parallel se crea un env de conda con hail y se vuelve a poner la verison 3.6
module load gcc
#04/09/2026:ana amil: 19/06/25 hace falta especifcar la version 1.9 de plink, la version 2.0 corresponde con plink2, que tiene otras funciones -> graci cambia y pone 1.9 en vez de a secas
module load plink/1.90
module load R/R
#04/09/2026: ana tiene esta siguiente linea comentada
source ~/.Renviron
...
#start added: 04/09/2026 
##ana amil 20/06/025 -> alargar el condicional para que no genere un archivo repeat*.gz. Crear el else para cambiar de nombre original_indivsample -> GUR: hace esto porque si no hay duplicados todo el rollo anterior no hace falta
else
	mv ${path_maf}/metadata/${date_dir}/original_indivsample.tsv ${path_maf}/metadata/${date_dir}/indivsample.tsv

fi
#end added: 04/09/2026
...
then
 ...
 # QUITAR COLUMNA GENOTIPO DE MIS SAMLES EXLCUIDOS Y QUITAR LA COLETILLA DEL REPEAT, DE LAS MUESTRAS QUE SE QUEDAN
  ...
  # 04/09/2026: ana amil 20/06/2025 -> poner condicional para evitar que busque repeats*.gz si no hay muestras repetidas -> linea de aqui abajo añadida
    if [[ -n "$duplicates" ]]; then
...
#04/09/2026: este fi ultimo añadido para cerrar el añadido de Ana if [[ -n "$duplicates" ]]; then -> se cierra por tanto el ----if [[ -n "$duplicates" ]]; then-----
# y el inicial -----if [[ $(cat ${path_maf}/tmp/plinkout/lista_muestras_excluidas.tsv | wc -l) == 0 ]]------
...
# 04/09/2026: ana amil 25/06/2025 -> no hace falta la version de bcftools 1.10, el error está en que no elimina el apartado FORMAT del header, por tanto tiene 9 elementos en la cabecera y 8 en el cuerpo de la tabla. Esta solucionado ya arriba
...
##04/09/2026: ana amil 20/06/2025 -> la ruta al hg38.fa era de tblab y no de la uam (/mnt/genetica7/references/hg38.fa)
