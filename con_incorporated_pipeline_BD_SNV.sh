...
# Data base path (sin barra final, la carpeta donde esta el coverage/, db/, imputed_vcf/, individualvcf/, merged_vcf/, metadata/)
path_maf=""

# TSV file with sample-pathology information (no se por que estoy pasando un .txt cuando normalmente se pasaba un .tsv, empece pasando un .tsv y ahora paso un .txt)
# es decir aqui hay que poner el path al archivo del metadata que esta dentro de la carpeta del metadata en .txt
mymetadatapathology_uniq="" #varias cat y varias subcat TODOS CES Y WGS Y WES


# Task directory: donde sea que esta el github
...
##### 1) HAGO 2 LISTAS: LA DE LOS VCFS YA INCORPORADOS (multisample.tsv) y LA DE LOS NUEVOS (ORIGINAL_INDIV_SAMPLE.TSV)

# NEW_VCFS= Iterate over all vcf.gz files in the specified directory -> EN LA CARPETA DE NUEVOS VCFS: HAGO UNA LISTA DE TODOS LOS QUE HAY NUEVOS Y LA LLAMO ORIGINAL_INDIV SAMPLES.TSV
...
### INCORPORATED_VCFS (los de la base de datos del 26/06/2024) -> multisample.tsv
for vcf in ${path_maf}/individual_vcf/incorporated_vcf/*.vcf.gz; do
        file_name=$(basename "$vcf" | cut -d '.' -f1)
        echo "$file_name" >> ${path_maf}/metadata/${date_dir}/multisample.tsv
done

### 2) DE LOS VCFS NUEVOS MIRAR CUANTOS CES, WGS Y WES HAY DE CADA TIPO Y PRIORIZAR. ESTO SOLO PARA LOS NUEVOS
